import { useGetBot, useUpdateBot, useDeleteBot, usePingBot, useListBotEvents, getGetBotQueryKey, getListBotEventsQueryKey, getListBotsQueryKey, getGetDashboardSummaryQueryKey } from "@workspace/api-client-react";
import { useRoute, useLocation } from "wouter";
import { StatusBadge } from "@/components/status-badge";
import { formatUptime, formatNumber, formatDate } from "@/lib/utils";
import { Layout } from "@/components/layout";
import { Terminal, Activity, Settings, Trash2, ArrowLeft, Zap, Save } from "lucide-react";
...
export default function BotDetail() {
  ...
  const handlePing = () => {
    pingBot.mutate({ id, data: { status: bot.status } }, {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getGetBotQueryKey(id) });
